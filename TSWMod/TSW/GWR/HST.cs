...
{
    class HST : ILocomotive
    {
        public const string NamePartial = "GWR_HST";
        ...
        public HST(Mem m, UIntPtr basePtr, IntPtr hWnd)
        {
            _m = m;
            _basePtr = basePtr;
            _hornButton = new HornLever(m,
                m.GetPtr(m.GetCodeRepresentation(basePtr + 0x04E8)));
            _throttle = new HSTThrottle(m, hWnd,
                m.GetPtr(m.GetCodeRepresentation(basePtr + 0x0698)));
            _brake = new HSTBrake(m, hWnd,
                m.GetPtr(m.GetCodeRepresentation(basePtr + 0x0690)));
        }

        public bool CheckPlayerCalibration()
        {
            if (!_hornButton.CurrentValue.AlmostEquals(0))
            {
                _hornUse += 1;
            }
        ...
        public void OnControlLoop(RailDriverLeverState state, int[] pressedButtons)
        {
            ...
            _brake.OnControlLoop(state.AutoBrakeTranslated);
        }
